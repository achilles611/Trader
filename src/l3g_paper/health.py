...
def sanitized_paper_health(status: Mapping[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "current_session_family": status.get("current_session_family", "OFF_SESSION"),
        "current_session_id": status.get("current_session_id"),
        "session_timezone": status.get("session_timezone", "America/New_York"),
        "session_definitions": status.get("session_definitions", []),
