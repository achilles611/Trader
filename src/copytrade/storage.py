...
class CopyTradeDatabase:
        ...
        with self._connect() as connection:
            connection.executescript(
                ...
                CREATE TABLE IF NOT EXISTS copy_candidate_scores (
                    ...
                    hard_gates_json TEXT NOT NULL DEFAULT '[]', score_version TEXT NOT NULL DEFAULT 'phase_b_suitability_v2',
                    PRIMARY KEY(target_wallet, calculated_at)
                );
            ...
            self._ensure_column(connection, "copy_candidate_scores", "score_version", "TEXT NOT NULL DEFAULT 'phase_b_suitability_v2'")
            ...
            self._ensure_column(connection, "copy_candidate_score_archive", "score_version", "TEXT NOT NULL DEFAULT 'phase_b_suitability_v2'")
            connection.execute(
                """INSERT OR IGNORE INTO copy_candidate_score_archive(
                   ...
                   SELECT rowid, ?, 'duplicate_phase_b_authority_migration', target_wallet, calculated_at, total_score,
                    component_scores_json, penalties_json, eligible, reasons_json, source_quality, provenance,
                    analysis_run_id, config_fingerprint, confidence_score, hard_gates_json, score_version
                   FROM copy_candidate_scores
    ...
    def analysis_funnel(self, run_id: str, *, high_suitability_score: float) -> dict[str, dict[str, float | int]]:
    ...
    def latest_scores(self) -> list[CandidateScore]:
        ...
        return [
            CandidateScore(target_wallet=row["target_wallet"], calculated_at=as_utc(row["calculated_at"]),
                           ...
                           hard_gates=tuple(_load(row["hard_gates_json"], [])), score_version=str(row["score_version"] or "phase_b_suitability_v2"))
            for row in rows
        ]
    ...
    def phase_b_qualified_scores(self, *, config_fingerprint: str | None = None) -> list[CandidateScore]:
        ...
        return [
            CandidateScore(
                ...
                score_version=str(row["score_version"] or "phase_b_suitability_v2"),
            )
            for row in rows
        ]
