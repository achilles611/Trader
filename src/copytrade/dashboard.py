...
<h2>Target candidates</h2><table id="targets"><tr><th>Wallet</th><th>Status</th><th>Account equity</th><th>Score</th><th>Eligible</th></tr></table>
...
function cell(v){const e=document.createElement('td');e.textContent=v;return e} function render(d){
 ...
 clear('targets'); const t=document.querySelector('#targets'); d.scores.forEach(s=>{const snap=d.target_snapshots.find(x=>x.target_wallet===s.target_wallet)||{};let r=document.createElement('tr');[s.target_wallet,(d.targets.find(x=>x.wallet===s.target_wallet)||{}).status||'unknown',dollar(snap.account_value),Number(s.total_score).toFixed(1),s.eligible?'yes':'no'].forEach(x=>r.append(cell(x)));t.append(r)});
