...
def score_candidate(
    ...
    *, source_quality: float = 1.0, diversification: float = 0.5, confidence_score: float = 0.0,
    ...
    if follower.regime_robustness is None:
    ...
    active_weights = {
        name: float(weight) for name, weight in config.score_weights.items()
        if name in components and float(weight) > 0
    }
    ...
    # Configured weights are relative.  Normalize only the components backed
    # by evidence, then express their contributions on a fixed 100-point
    # scale.  Missing evidence can still produce an exploratory estimate, but
    # can never inflate the raw scale above 100; finalist policy handles the
    # evidence-strength decision separately via confidence and explicit gates.
    weighted = _normalized_component_scores(components, active_weights)
    ...
    soft_reasons = {
        ...
        "regime_unavailable", "jackpot_concentration",
    }
...
def _normalized_component_scores(
    components: Mapping[str, float], active_weights: Mapping[str, float],
) -> dict[str, float]:
    """Return fixed-scale component contributions for relative configured weights."""
    total_weight = sum(max(float(weight), 0.0) for weight in active_weights.values())
    if total_weight <= 0:
        return {}
    return {
        name: 100.0 * _clamp01(float(components[name])) * max(float(weight), 0.0) / total_weight
