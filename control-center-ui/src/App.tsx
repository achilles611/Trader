...
function ToastNotice({ toast, dismiss }: { toast: Toast; dismiss: () => void }) {
  ...
  return <div className={`toast ${toast.tone}`} role="alert"><span className="toast-title">{toast.tone === "error" ? "Action failed" : toast.tone === "warning" ? "Warning" : "Updated"}</span><span className="toast-message">{toast.message}</span><button onClick={dismiss} aria-label="Dismiss notification">×</button></div>;
}
...
export function App() {
  ...
  if (consoleMode === "slim") return <><SlimConsole paper={paperConsole} onFullConsole={() => selectConsoleMode("full")} /><ToastNotice toast={toast} dismiss={() => setToast(null)} /></>;
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <section className="paper-banner"><strong>PAPER POSITIONS ONLY</strong><span>Research and simulated execution only. No live credentials, live signing, live order submission, or live-mode controls exist in this application.</span></section>
    ...
    <ToastNotice toast={toast} dismiss={() => setToast(null)} />
...
function LaneIIIPaperPage({ paper, confirmation }: { paper: PaperConsoleState; confirmation: (value: Confirmation) => void }) {
  ...
  return <div className="page-grid">
    ...
    <section className="panel span-12"><PanelTitle title="Operational paper startup diagnostics" subtitle="The active start and profile-switch stages remain visible while Scalper waits for genuine current-session evidence." /><MetricList values={[["Auto-start stage", autoStart.stage || "IDLE"], ["Profile-switch stage", profileSwitch.stage || "IDLE"], ["Target profile", profileSwitch.target_profile || status?.entry_profile_version || "UNAVAILABLE"], ["Evidence covered", autoWarmup.covered_family_count == null ? "—" : `${autoWarmup.covered_family_count} / 3`], ["Missing evidence", autoWarmup.missing_families?.length ? autoWarmup.missing_families.join(", ") : "NONE"], ["Warmup elapsed", typeof autoWarmup.elapsed_seconds === "number" ? `${autoWarmup.elapsed_seconds}s / ${autoWarmup.timeout_seconds ?? "?"}s` : "—"], ["Full verifier", autoStart.ledger?.status || "UNVERIFIED"], ["Verified through", autoStart.ledger?.verified_through_sequence ?? "—"]]} />{autoStart.blockers?.length > 0 && <ReasonList label="Exact startup blockers" values={autoStart.blockers} />}{profileSwitch.blockers?.length > 0 && <ReasonList label="Exact profile-switch blockers" values={profileSwitch.blockers} />}</section>
    ...
    <div className="l3g-controls"><button className="button minor" disabled={busy} onClick={() => void runRehearsal()}>Run Read-Only Commissioning Rehearsal</button><button className="button positive" disabled={busy || !rehearsalIsCurrent} onClick={() => confirmation({ title: "Start the exact commissioning lifecycle?", body: "One atomic request revalidates every gate, reserves commissioning ownership, and waits for the first profile-qualified signal before submitting one sealed 1 MNQ entry.", confirm: "Arm Profile Commissioning", action: startCommissioning })}>Atomic Commissioning Start</button></div></section>
    ...
    <section className="panel span-6"><PanelTitle title="NinjaTrader observer maintenance diagnostics" subtitle="Recovery-only hard reset. It preserves restart gates, requests graceful shutdown, and cannot arm, submit, cancel, flatten, or force-kill." /><MetricList values={[["Workflow stage", maintenance.stage || "IDLE"], ["Process", maintenance.process?.state || "UNKNOWN"], ["Control Center", maintenance.process?.control_center_detected ? "DETECTED" : "NOT DETECTED"], ["Automatic sign-in", maintenance.login?.state || "NOT_CONFIGURED"], ["AddOn", maintenance.addon?.state || "UNSTARTED"], ["AddOn provenance", maintenance.addon?.provenance || "UNVERIFIED"], ["Configured instrument", maintenance.configured_instrument || "UNRESOLVED"], ["Observer mode", maintenance.observer?.subscription_mode || "UNVERIFIED"], ["Observer attached", maintenance.observer?.attached ? "YES" : "NO"], ["Market data", maintenance.observer?.market_data_fresh ? "FRESH" : maintenance.observer?.freshness_reason || "NOT FRESH"], ["Blockers", maintenance.blockers?.length ? maintenance.blockers.join(", ") : "NONE"], ["Manual action", maintenance.manual_action || "NONE"], ["Launches", maintenance.actions?.launches ?? 0], ["Graceful shutdowns", maintenance.actions?.graceful_shutdowns ?? 0], ["Forced shutdowns", maintenance.actions?.forced_shutdowns ?? 0], ["Historical command baseline", maintenance.actions?.execution_command_baseline ?? 0], ["Commands sent by maintenance", maintenance.actions?.execution_commands_sent_by_task ?? 0], ["Orders submitted / cancelled", `${maintenance.actions?.orders_submitted_by_task ?? 0} / ${maintenance.actions?.orders_cancelled_by_task ?? 0}`], ["Latest diagnostic", maintenance.diagnostics?.length ? maintenance.diagnostics[maintenance.diagnostics.length - 1].message : "NONE"]]} /><button className="button warning" disabled={busy || maintenance.process?.state !== "RUNNING" || maintenance.in_progress === true || maintenance.button?.enabled !== true} onClick={() => confirmation({ title: "Hard reset NinjaTrader and repair the observer?", body: "This recovery action re-verifies the disarmed, flat, order-free, reconciled runtime and zero ledger tail before requesting a graceful NinjaTrader restart. It never force-terminates the process.", confirm: "Hard Reset NinjaTrader + Observer", action: paper.startNinjaTraderMaintenance })}>Hard Reset NinjaTrader + Observer</button></section>
    <section className="panel span-6"><PanelTitle title="Session regime" subtitle="Asia, London/Europe, New York RTH, and NY After have exact independent session identity and evidence lifecycle." /><MetricList values={[["Current session", status?.current_session || "OFF_SESSION"], ["Session family", status?.current_session_family || "OFF_SESSION"], ["Current session ID", status?.current_session_id || "—"], ["Trade date", status?.trade_date || "—"], ["Session state", status?.session_state || "—"], ["Session timezone", status?.session_timezone || "America/New_York"], ["Entry window", status?.entry_window || "—"], ["Entry cutoff", status?.entry_cutoff || "—"], ["Hard flat", `${status?.hard_flat_deadline || "—"} ${status?.session_timezone || "America/New_York"}`], ["Session arm", status?.session_armed_state || "DISARMED"], ["Commissioning warmup", warmup.status || "NOT_WARMED"], ["Strategy evidence", status?.strategy_evidence_status || "INCOMPLETE"]]} /></section>
    <section className="panel span-6"><PanelTitle title="Paper risk" subtitle="Account loss is continuous across every profile; entry caps and loss streaks belong to the selected strategy." /><MetricList values={[["Account session P&L", money(status?.session_pnl)], ["Selected-profile session P&L", money(status?.profile_session_pnl)], ["Asia account P&L", money(status?.asia_session_pnl)], ["London account P&L", money(status?.london_session_pnl)], ["New York account P&L", money(status?.new_york_session_pnl)], ["Account trade-date P&L", money(status?.combined_trade_date_pnl ?? status?.daily_realized_pnl)], ["Account loss allowance remaining", money(status?.combined_trade_date_loss_allowance_remaining)], ["Selected-profile session entries", status?.session_entries ?? 0], ["Selected-profile trade-date entries", status?.profile_trade_date_entry_count ?? status?.trade_date_entry_count ?? 0], ["Account trade-date entries", status?.account_trade_date_entry_count ?? 0], ["Selected-profile consecutive losses", status?.consecutive_losses ?? 0], ["Maximum position", "1 MNQ"], ["Account daily loss limit", "$200"]]} /></section>
    <section className="panel span-6"><PanelTitle title="Latest provisional inference" /><MetricList values={[["Entry profile", status?.entry_profile || "UNAVAILABLE"], ["Profile version", status?.entry_profile_version || "—"], ["Entry session", status?.entry_session_kind || "—"], ["Effective threshold", status?.effective_confidence_threshold || "—"], ["Dominance margin", status?.entry_dominance_margin || "—"], ["Required families", status?.entry_family_count ?? "—"], ["Maximum hold", status?.maximum_position_age_seconds === undefined ? "—" : `${status.maximum_position_age_seconds}s`], ["Maximum entries", status?.maximum_session_entries ?? "—"], ["Data quality", policy.quality || "UNUSABLE"], ["Classification", commissioning.classification || "STRATEGY_GENERATED_PAPER"], ["Last decision", lastDecision.decision || "—"], ["Hypothesis", lastDecision.hypothesis_kind || "—"], ["Relative support", lastDecision.relative_support || "—"], ["Reason", lastDecision.reason_code || "—"], ["Policy hash", status?.authority?.paper_policy_hash || "—"], ["Risk profile hash", status?.authority?.risk_profile_hash || "—"], ["Scientific eligibility", "False"]]} /></section>
