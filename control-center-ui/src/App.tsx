...
function LaneIIIPaperPage({ paper, confirmation }: { paper: PaperConsoleState; confirmation: (value: Confirmation) => void }) {
  ...
  return <div className="page-grid">
    ...
    <section className="panel span-6"><PanelTitle title="Latest provisional inference" /><MetricList values={[["Entry profile", status?.entry_profile || "UNAVAILABLE"], ["Profile version", status?.entry_profile_version || "—"], ["Effective threshold", status?.effective_confidence_threshold || "—"], ["Dominance margin", status?.entry_dominance_margin || "—"], ["Required families", status?.entry_family_count ?? "—"], ["Re-entry cooldown", status?.reentry_cooldown_seconds === undefined ? "—" : `${status.reentry_cooldown_seconds}s`], ["Data quality", policy.quality || "UNUSABLE"], ["Classification", commissioning.classification || "STRATEGY_GENERATED_PAPER"], ["Last decision", lastDecision.decision || "—"], ["Hypothesis", lastDecision.hypothesis_kind || "—"], ["Relative support", lastDecision.relative_support || "—"], ["Reason", lastDecision.reason_code || "—"], ["Policy hash", status?.authority?.paper_policy_hash || "—"], ["Risk profile hash", status?.authority?.risk_profile_hash || "—"], ["Scientific eligibility", "False"]]} /></section>
