...
export function App() {
  ...
  const closeAll = async (path: string, success: string) => {
    try {
      ...
      if (result.status === "partial") setToast({ tone: "warning", message: `Partial close-all: ${result.closed?.length || 0} groups closed; ${result.skipped?.length || 0} skipped. New PAPER entries remain paused.` });
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      {page === "Candidates" && <CandidatesPage notify={setToast} confirmation={setConfirmation} refresh={refresh} />}
...
function CandidatesPage({ notify, confirmation, refresh }: { notify: (toast: Toast) => void; confirmation: (value: Confirmation) => void; refresh: () => Promise<void> }) {
