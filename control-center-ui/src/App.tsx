...
function LaneIIIPaperPage({ paper, confirmation }: { paper: PaperConsoleState; confirmation: (value: Confirmation) => void }) {
  ...
  const entryProfile = status?.entry_profile || {}; const beeztmodeActive = entryProfile.selected_profile === "BEEZTMODE_V1";
  ...
  return <div className="page-grid">
    ...
    <section className={`panel span-6 experimental-profile-panel ${beeztmodeActive ? "active" : ""}`}><PanelTitle title="Experimental entry profile" subtitle="Backend-owned Sim101 sampling policy; all safety and risk gates remain unchanged." /><div className="experimental-profile-title"><strong>Beeztmode</strong><span className={`beeztmode-badge ${beeztmodeActive ? "active" : "standard"}`}>{beeztmodeActive ? "BEEZTMODE ACTIVE" : "STANDARD"}</span></div><MetricList values={[["Selected profile", entryProfile.selected_profile || "STANDARD"], ["Standard threshold", entryProfile.standard_threshold ?? "—"], ["Effective threshold", entryProfile.effective_threshold ?? "—"], ["Current / recent candidate confidence", entryProfile.current_candidate_confidence ?? "—"], ["Distance above / below threshold", entryProfile.distance_to_active_threshold ?? "—"], ["Last candidate disposition", entryProfile.last_candidate_disposition ?? "—"], ["Last rejection reason", entryProfile.last_rejection_reason ?? "—"], ["Profile activation timestamp", entryProfile.activation_timestamp ? timeLabel(entryProfile.activation_timestamp) : "—"], ["Profile version", entryProfile.entry_profile_version || "—"], ["Paper only", entryProfile.paper_only === true ? "Yes" : "STALE"]]} /><button className="button beeztmode-control" disabled={busy || entryProfile.selection_permitted !== true} onClick={() => beeztmodeActive ? void paper.selectEntryProfile("STANDARD") : confirmation({ title: "Enable Beeztmode?", body: "Beeztmode lowers the paper entry-confidence threshold and may increase trade frequency. All safety and risk gates remain active.", confirm: "Enable Beeztmode", action: async () => { await paper.selectEntryProfile("BEEZTMODE_V1"); } })}>{beeztmodeActive ? "Use Standard" : "Enable Beeztmode"}</button>{entryProfile.selection_permitted !== true && <p className="muted">{entryProfile.selection_reason || "Profile selection is unavailable."}</p>}</section>
