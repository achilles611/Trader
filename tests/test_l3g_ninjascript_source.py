...
class NinjaScriptSourceTests(unittest.TestCase):
    ...
    def test_callbacks_require_exact_instrument_and_preowned_order_reference(self) -> None:
        ...
        self.assertIn("eventState == OrderState.Initialized", order_callback)
        self.assertIn("eventFilled == 0", order_callback)
        self.assertIn("eventQuantity == MaximumQuantity", order_callback)
        self.assertIn("if (!preSubmitInitialization)", order_callback)
